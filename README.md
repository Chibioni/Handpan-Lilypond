...
| `\|`        | 小節線チェック                                      | LilyPondの `\|` に相当するバーチェック |
| `~`         | タイ（次の同一音高の音符と弧線で結ぶ）              | LilyPondの `~` に相当するタイ          |

ハンドパンは非クロマティック楽器のため、音符ごとの臨時記号（#・b）は使用しない。
スケールのフラット・シャープ構成は楽譜冒頭の**調号**と**スケール名**（タイトル）で示す。
...
- [midiToHandpanScore](midiToHandpanScore/README.md) — MIDI ファイルをハンドパン記法の LilyPond ファイルに変換するツール
