import pytest
from miditohandpanscore.models import _compute_ly_pitches, HandpanScale, HandpanPart, HandpanSet
...
class TestComputeLyPitches:
    def test_ding_is_minus_one(self):
        result = _compute_ly_pitches(KURD9_MIDIS)
        assert result[0] == -1
    ...
    def test_notes_below_ding(self):
        ...
        midis = [62, 61]
        assert _compute_ly_pitches(midis) == [-1, -2]

    def test_mixed_order_preserved(self):
        # 入力順が出力に反映される
        midis = [62, 67, 60]  # D4(ding), G4, C4
        result = _compute_ly_pitches(midis)
...
class TestHandpanSet:
    ...
    def handpan_set(self):
        ...
        return HandpanSet(
            scale_family="Kurd",
            parts=[HandpanPart("Left", scale_a), HandpanPart("Right", scale_b)],
            key_signature="2b",
        )
